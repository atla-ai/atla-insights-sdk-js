...
export function instrumentLangChain(callbackManagerModule?: any): void {
  ...
  if (callbackManagerModule) {
    try {
      ...
      diag.warn("Failed to manually instrument LangChain callback manager", e as Error);
    }
  } else {
    ...
    try {
      ...
      diag.debug("@langchain/core not found; LangChain will still run, but callbacks won't be auto-instrumented.");
    }
  }
