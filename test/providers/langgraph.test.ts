...
describe("langgraph", () => {
  ...
  const { setOpenAIMockResponse, resetOpenAIMock } = require("../__mocks__/openai");
  ...
  it("instruments a simple LangGraph workflow", async () => {
    ...
    const llmSpans = finishedSpans.filter(span => span.attributes[SemanticConventions.OPENINFERENCE_SPAN_KIND] === OpenInferenceSpanKind.LLM);
  ...
  it("instruments a react agent with a tool", async () => {
    ...
    setOpenAIMockResponse((args: any) => {
      if (args?.tools && args.messages?.some((m: any) => 
    ...
    const toolSpans = spans.filter(s => s.attributes[SemanticConventions.OPENINFERENCE_SPAN_KIND] === OpenInferenceSpanKind.TOOL);
